...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      {/* nav bar  */}
      <nav className="relative overflow-visible w-full flex items-center lg:justify-between justify-between gap-10 lg:gap-0 lg:p-4 p-2 bg-[#00000093] rounded-3xl mt-6 sticky top-4 z-50 ">
        {/* left side */}
        ...
        <div className="hidden lg:flex gap-4">
          ...
          <Link href="/auth/register"><button className="text-sm text-white bg-gradient-to-r from-blue-500 via-purple-500 to-cyan-500 px-3 py-2 rounded-2xl cursor-pointer hover:scale-[1.040] transition-all ease-in-out duration-300">Get Started</button></Link>

        </div>

        <button type="button" className="bg-black rounded-full w-fit p-2 lg:hidden cursor-pointer" onClick={() => setIsNavbarOpen(!isNavbarOpen)}> <Menu className="text-white w-6 h-6" /> </button>

        {/* mobile nav  */}
        <ul className={`flex flex-col gap-5 absolute top-full left-0 right-0 w-full bg-[#00000093] rounded-3xl mt-2 p-4 transition-all duration-300 ease-in-out lg:hidden z-50 ${isNavbarOpen ? "opacity-100 visible pointer-events-auto" : "opacity-0 invisible pointer-events-none"}`}>
          <li className="text-neutral-500 text-[13px] hover:text-white transition-all duration-300 ease-in-out cursor-pointer">Home</li>
